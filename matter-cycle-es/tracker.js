// Rastreador de Partículas de Materia — Utah SEEd 5.3.3
'use strict';

// ── Tracker strings (Spanish only) ────────────────────────────────────────────
const TT = {
  particleLbl:'✨ Materia',
  breathLbl:'exhalada',
...
const STOPS = [
  {step:0, x:260, y:110, cpx:260, cpy: 60, msg:'¡La materia está en el aire y el suelo!'},
  {step:1, x: 94, y:260, cpx:130, cpy: 55, msg:'¡El cactus toma materia del aire y agua para crecer!'},
  {step:2, x:287, y:268, cpx:200, cpy:195, msg:'¡La rata come el cactus — la materia pasa a la rata!',         breath:true},
  {step:3, x:385, y:275, cpx:325, cpy:210, msg:'¡La serpiente come la rata — la materia pasa a la serpiente!', breath:true},
  {step:4, x:165, y:276, cpx:265, cpy:318, msg:'¡La serpiente muere — pero la materia sigue aquí!'},
  {step:5, x:228, y:318, cpx:210, cpy:293, msg:'¡Los descomponedores descomponen la materia muerta!',          decompose:true},
...
function drawParticle(){
  ...
  const lw=trkCx.measureText(TT.particleLbl).width;
  trkCx.fillStyle='rgba(20,8,0,0.75)';
  trkCx.beginPath(); trkCx.roundRect(trkX-lw/2-7,trkY-40,lw+14,18,5); trkCx.fill();
  trkCx.fillStyle='#fff'; trkCx.fillText(TT.particleLbl,trkX,trkY-31);
  trkCx.restore();
...
function spawnBreath(){
  for(let i=0;i<4;i++) extras.push({
    ...
    a:.85, da:.008, r:4, color:'#aed6f1', lbl:i===0?TT.breathLbl:null
  });
}
...
function finishCycle(){
  ...
  msgEl.innerHTML=`<p>${TT.finishMsg}</p>
    <button class="follow-btn" id="watchAgainBtn">${TT.watchAgain}</button>`;
...
function updateControls(){
  ...
  document.getElementById('prevBtn').textContent=TT.prevBtn;
  document.getElementById('nextBtn').textContent=TT.nextBtn;
  document.getElementById('stepCount').textContent=`${TT.stopLabel} ${trkStop+1} ${TT.of} ${STOPS.length}`;
}
