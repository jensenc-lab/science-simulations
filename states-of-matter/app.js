...
// ── Substance data ────────────────────────────────────────────────────────────
const SUBS = {
  water:  { name:'Water (H₂O)', color:'#3498db', mp:0,    bp:100,  tMin:-50,  tMax:150,  t0:-30  },
  iron:   { name:'Iron (Fe)',   color:'#95a5a6', mp:1538, bp:2862, tMin:1400, tMax:3000, t0:1400 },
  oxygen: { name:'Oxygen (O₂)',color:'#1abc9c', mp:-218, bp:-183, tMin:-230, tMax:-160, t0:-225 }
};

// ── State ─────────────────────────────────────────────────────────────────────
const N = 40, COLS = 8, ROWS = 5, SP = 14, RAD = 6;
let subKey = 'water', sub = SUBS.water, temp = sub.t0;
...
function initParticles() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (state === 'solid') {
      ...
      x = LEFT + RAD + Math.random() * (RIGHT - LEFT - RAD * 2);
      y = CEIL + RAD + Math.random() * (FLOOR - CEIL - RAD * 2);
...
function updateParticles() {
  ...
  if (state === 'solid') {
    ...
    particles.forEach(p => {
      ...
      p.x += (p.hx - p.x) * 0.35 * transPhase + p.vx;
      p.y += (p.hy - p.y) * 0.35 * transPhase + p.vy;
    });
  } else if (state === 'liquid') {
    const liqCeil = FLOOR - (FLOOR - CEIL) * 0.44;
    const spd = 0.9 + hf * 1.8;
    particles.forEach(p => {
      p.vx += (Math.random() - .5) * 0.38; p.vy += (Math.random() - .5) * 0.38;
      const v = Math.hypot(p.vx, p.vy);
      if (v > spd) { p.vx = p.vx / v * spd; p.vy = p.vy / v * spd; }
      p.x += p.vx; p.y += p.vy;
    ...
    particles.forEach(p => {
      ...
      p.vx = p.vx / v * spd; p.vy = p.vy / v * spd;
      p.x += p.vx; p.y += p.vy;
      if (p.x < LEFT + RAD)   { p.x = LEFT + RAD;   p.vx =  Math.abs(p.vx); }
      if (p.x > RIGHT - RAD)  { p.x = RIGHT - RAD;  p.vx = -Math.abs(p.vx); }
...
function drawFrame() {
  ...
  particles.forEach(p => {
    ...
    cx.fillStyle = g;
    cx.beginPath(); cx.arc(p.x, p.y, RAD, 0, Math.PI * 2); cx.fill();
  });
  // State watermark
  cx.fillStyle = 'rgba(255,255,255,0.1)';
  cx.font = 'bold 12px Segoe UI,system-ui,sans-serif';
  cx.textAlign = 'right'; cx.fillText(state.toUpperCase(), RIGHT - 6, CEIL + 16);
}
...
function updateUI() {
  const s = sub, cfg = STATE_CFG[state] || STATE_CFG.solid;
  const pct = Math.max(0, Math.min(100, (temp - s.tMin) / (s.tMax - s.tMin) * 100));
  document.getElementById('thermoFill').style.height = pct + '%';
  // Thermometer marks
  const mpPct = (s.mp - s.tMin) / (s.tMax - s.tMin) * 100;
  const bpPct = (s.bp - s.tMin) / (s.tMax - s.tMin) * 100;
...
function setupSlider() {
  ...
  document.getElementById('mpMarkerLbl').textContent = sub.mp + '°C MP';
  document.getElementById('bpMarkerLbl').textContent = sub.bp + '°C BP';
  document.getElementById('mpMarkLbl').textContent   = 'MP ' + sub.mp + '°C';
  document.getElementById('bpMarkLbl').textContent   = 'BP ' + sub.bp + '°C';
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setupSlider(); initParticles(); loop();
});
