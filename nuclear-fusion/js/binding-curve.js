...
(function () {
  ...
  function buildSvg(data) {
    svg = svgEl('svg', {
      ...
      role: 'img',
      'aria-label':
        'Binding energy per nucleon versus mass number. Curve rises sharply from hydrogen to iron, peaks near iron-56, and gradually falls toward uranium.',
    });
  ...
  function drawPoints(data) {
    ...
    data.forEach((n, i) => {
      ...
      const circle = svgEl('circle', {
        ...
        role: 'button',
        'aria-label':
          n.name + '. ' + n.protons + ' protons, ' + n.neutrons + ' neutrons. ' +
          'Binding energy ' + n.bePerNucleon.toFixed(3) + ' MeV per nucleon. ' +
          'Activate to select.',
        'data-idx': String(i),
  ...
  function showTooltip(entry) {
    ...
    tooltipEl.innerHTML =
      ...
      '<div class="bc-tooltip__row">' + n.protons + ' protons · ' + n.neutrons + ' neutrons</div>' +
  ...
  function selectNuclide(n) {
    if (!infoEl) return;
    let html = '<h3>Selected Nuclide</h3>';
    html +=
      '<dl class="binding-curve__info-grid">' +
        '<dt>Name</dt><dd>' + escapeHtml(n.name) + '</dd>' +
        '<dt>Symbol</dt><dd><sup>' + n.mass + '</sup>' + escapeHtml(n.symbol) + '</dd>' +
        '<dt>Composition</dt><dd>' + n.protons + ' protons, ' + n.neutrons + ' neutrons</dd>' +
        '<dt>Binding Energy</dt><dd>' + n.bePerNucleon.toFixed(3) + ' MeV/A</dd>' +
      '</dl>';
    if (n.notes) {
  ...
  function showFusionPath(reactants, product) {
    ...
    const word = isMoreStable ? 'released' : 'required';
