...
// Reads shared globals from app.js: ctx, W, H, systems, animT, S
...
function drawWeatherHud() {
  ...
  const lbl  = wxTlRunning ? S.tlDay(wxTlDay) : S.tlDone;
...
function startTimeLapse() {
  ...
  function advance() {
    if (wxTlDay >= 3) {
      ...
      document.getElementById('infoText').textContent = S.tlSummary(d1, d2, d3);
      setTimeout(() => { wxTlDay = 0; }, 4000);
      return;
    }
    setTimeout(() => {
      wxTlDay++;
