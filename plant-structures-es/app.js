// Plant Structures Explorer — Utah SEEd 4.1.1 — Spanish only
'use strict';
...
// ── State ──────────────────────────────────────────────────────────────────────
const lang = 'es';
let currentPart = null, currentEnv = 'desert';
...
function buildDesertScene() {
  ...
  const t = T[lang].desert.labels;
  const labels = `
    <text id="lbl-roots"  x="371" y="393" ${lf} fill="#27ae60">${t.roots}</text>
    <text id="lbl-pads"   x="373" y="258" ${lf} fill="#c07010">${t.pads}</text>
    <text id="lbl-spines" x="196" y="210" ${lf} fill="#e74c3c">${t.spines}</text>
    <text id="lbl-flower" x="${fx+18}" y="${fy+4}" ${lf} fill="#c0185e">${t.flower}</text>
    <text id="lbl-fruit"  x="456" y="188" ${lf} fill="#8e1244">${t.fruit}</text>`;
...
function buildForestScene() {
  ...
  const lf = `font-size="13" font-weight="800" font-family="Segoe UI,system-ui,sans-serif"`;
  const t = T[lang].forest.labels;
  const labels = `
    <text id="lbl-roots"   x="371" y="387" ${lf} fill="#1a8040">${t.roots}</text>
    <text id="lbl-bark"    x="369" y="235" ${lf} fill="#a0703a">${t.bark}</text>
    <text id="lbl-leaves"  x="450" y="99"  ${lf} fill="#2a8020">${t.leaves}</text>
    <text id="lbl-flowers" x="316" y="166" ${lf} fill="#3a2510" filter="url(#tGlow)">${t.flowers}</text>
    <text id="lbl-sprouts" x="173" y="296" ${lf} fill="#2a9010">${t.sprouts}</text>`;
...
function buildWetlandScene() {
  ...
  const lf = `font-size="13" font-weight="800" font-family="Segoe UI,system-ui,sans-serif"`;
  const t = T[lang].wetland.labels;
  const labels = `
    <text id="lbl-roots"    x="371" y="387" ${lf} fill="#27ae60">${t.roots}</text>
    <text id="lbl-stem"     x="364" y="203" ${lf} fill="#2a7a10">${t.stem}</text>
    <text id="lbl-leaves"   x="222" y="254" ${lf} fill="#2a7a10">${t.leaves}</text>
    <text id="lbl-seedhead" x="374" y="98"  ${lf} fill="#6a3010">${t.seedhead}</text>
    <text id="lbl-seeds"    x="226" y="86"  ${lf} fill="#666666">${t.seeds}</text>`;
